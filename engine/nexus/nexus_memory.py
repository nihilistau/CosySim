    ...
    def remember(
        ...
        try:
            ...
            if entry_id:
                ...
                logger.debug("NexusMemory: stored for %s: %s", self._agent_id, content[:50])
                return entry_id
    ...
    def recall(
        ...
        try:
            ...
            r = requests.get(
                ...
                params={"q": search_query, "limit": top_k * 2},
                timeout=5,
            )
            if not r.ok:
                return []

            results = r.json().get("data", r.json()) if r.ok else []
            ...
            logger.warning("NexusMemory: recall failed: %s", exc)
            return []
    ...
    def get_context_window(
        ...
        try:
            r = requests.get(
                f"{self._url}/api/search",
                params={
                    "q": f"agent:{self._agent_id} memory",
                    "limit": 10,
                },
                timeout=5,
            )
