...
from engine.nexus.models import NLMNotebook, NLMAnswer, NLMSource
...
class NLMEngine:
    ...
    def __init__(self) -> None:
        cfg = get_config()
        self._proxy_url: str = cfg.get("notebooklm.base_url", "http://localhost:8800")
    ...
    def create_notebook(
        self, name: str, sources: Optional[List[str]] = None
    ) -> Dict[str, Any]:
    ...
    def list_notebooks(self) -> List[NLMNotebook]:
        ...
        Returns:
            List of NLMNotebook models.
        """
        # Try proxy first
        result = self._get_any("/notebooks")
        if isinstance(result, dict):
            notebooks_data = result.get("notebooks", result.get("data", []))
            if isinstance(notebooks_data, list):
                return [
                    NLMNotebook(**nb)
                    for nb in notebooks_data
                    if isinstance(nb, dict) and "id" in nb
                ]
            return []
    ...
    def ask(self, notebook_id: str, question: str) -> Union[NLMAnswer, Dict[str, Any]]:
        ...
        Returns:
            NLMAnswer model on success, or Dict with error info.
        """
        self._stats.asks += 1
        self._stats.total_questions += 1

        # Try proxy endpoint
        result = self._post_any(
            "/ask",
            {
                "notebook_id": notebook_id,
                "question": question,
        ...
        # Map typical proxy responses to standard model
    ...
    def converse(
        self, notebook_id: str, message: str, session_id: str = ""
    ) -> Dict[str, Any]:
    ...
    def _try_get(
        self, base_url: str, path: str, params: Optional[Dict[str, str]] = None
    ) -> Any:
    ...
    def _get_any(self, path: str, params: Optional[Dict[str, str]] = None) -> Any:
