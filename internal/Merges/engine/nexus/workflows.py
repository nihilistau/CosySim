    ...
    def generate_greetings(
        ...
        for mood, greetings_list in moods.items():
            ...
            entry_id = self._store(
                ...
                content,
                ["content", f"character:{character_id}", f"mood:{mood}", "greetings"]
                + p_tags,
            )
            if entry_id:
                ids.append(entry_id)
        ...
        logger.info(
            "ContentWorkflow: generated %d greeting sets for %s", len(ids), character_id
        )
        return ids
    ...
    def research(
        ...
        if search_results:
            result["sources"].append(
                {"type": "fts_search", "count": len(search_results)}
            )
            # Synthesize from search results
            snippets = []
            for e in search_results:
                if e and getattr(e, "content", None):
                    content_str = str(getattr(e, "content", ""))
                    snippets.append(content_str[:200])
            if snippets:
                result["answer"] = (
...
class NotebookWorkflow:
    ...
    def seed_notebook_knowledge(self, notebook_id: str = "all") -> Dict[str, Any]:
        ...
        seeds = (
            self.NOTEBOOK_SEEDS
            if notebook_id == "all"
            else {notebook_id: self.NOTEBOOK_SEEDS.get(notebook_id, {})}
        )
        ...
        for nb_id, seed in seeds.items():
            ...
            try:
                ...
                logger.debug("Failed to seed notebook knowledge", exc_info=True)
    ...
    def _generate_qa(
        ...
        topic_qa = {
            "mcp": (
                ...
                "MCPFramework is the central state tree and tool registry. It manages scene nodes, character nodes, timers, and provides the singleton pattern via get_framework().",
            ),
            "interceptors": (
                "How does the interceptor pipeline work?",
                "InterceptorPipeline wraps LLM calls with pre/post hooks. 25 interceptors run in priority order (lower first). Pre-call modifies system prompt/messages, post-call processes the response.",
            ),
            "scenes": (
                "How do CosySim scenes work?",
                "Scenes inherit from BaseScene, override start/stop/get_plugin_info. Each scene has MCP nodes, skills, templates, and Flask routes. SceneManager handles lifecycle.",
            ),
            "nexus": (
                "What is Nexus KMS?",
                "Nexus is the central knowledge management system. It provides FTS5 search, Q&A caching, rules engine, research sessions, and NotebookLM integration. API on port 8700.",
            ),
            "skills": (
                "How does the skill system work?",
                "Skills use the @skill decorator with pack, description, category, cooldown, cost. They're registered in SKILL_REGISTRY and exposed to agents via SkillAwarenessInterceptor.",
            ),
            "agents": (
                "How do CosySim agents work?",
                "VirtualAgent handles LLM inference. CharacterAgent adds personality. AgentGovernor manages the interceptor pipeline. Governance context flows through the chain.",
            ),
        }
