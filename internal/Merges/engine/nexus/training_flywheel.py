...
class TrainingFlywheel:
        ...
        with self._lock:
            with self._connect() as conn:
                try:
                    ...
                    logger.debug("Duplicate example skipped — hash=%s", content_hash)
                    return ""
    ...
    def collect_from_qa(
        ...
        eid = self._store_example(example)
        if eid:
            logger.info("Collected Q&A example — id=%s source=%s", eid[:8], source)
        return eid
    ...
    def export_jsonl(
        ...
        """
        rows = self._query_for_export(
            min_quality, source_filter, exclude_source="preference"
        )
        if not rows:
            ...
            for entry in entries:
                ...
                for tag in entry.tags:
                    if tag.startswith("quality:"):
                        try:
                            confidence = float(tag.split(":")[1]) / 10.0
                        except ValueError:
                            pass
