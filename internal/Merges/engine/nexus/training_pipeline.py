...
class TrainingPipeline:
    ...
    def capture_interaction(
        ...
        client = get_nexus_client(self._url)
    ...
    def export_dataset(
        ...
        try:
            results = client.search(
                query=f"dataset:{dataset_type} training",
                limit=500,
            )
    ...
    def generate_synthetic(
        ...
        if dataset_type == "tag_extraction":
            templates = [
                (
                    "How are you feeling?",
                    '*smiles warmly* [ACTION:twirls hair] "I\'m feeling great today!" [MOOD:happy] [STAT:trust+2]',
                ),
                (
                    "Tell me about yourself",
                    '*leans back* [ACTION:crosses legs] "Well, I\'m quite the adventurer..." [MOOD:confident]',
                ),
                (
                    "I missed you",
                    '*blushes* [ACTION:looks down shyly] "I... I missed you too." [MOOD:bashful] [STAT:affection+5]',
                ),
                (
                    "Want to play a game?",
                    '*eyes light up* [ACTION:claps hands] "Oh yes! What kind of game?" [MOOD:excited] [STAT:engagement+3]',
                ),
                (
                    "That was rude",
                    '*frowns* [ACTION:crosses arms] "I didn\'t mean it that way..." [MOOD:defensive] [STAT:trust-3]',
                ),
            ]
