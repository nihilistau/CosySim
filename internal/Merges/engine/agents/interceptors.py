...
class InterceptorRegistry:
    """
    Dynamic auto-registry for all available interceptors.

    Discovers all non-abstract subclasses of InterceptorBase across the codebase
    and instantiates them for the default pipeline.
    """

    @classmethod
    def discover_and_instantiate(cls) -> List[InterceptorBase]:
        import inspect
        from engine.mcp.comms_types import InterceptorBase

        # Ensure modules with extra interceptors are loaded
        try:
            import engine.agents.dialogue_gate
            import engine.characters.memory
            import engine.characters.reputation
            import engine.content.content_gate
            import engine.world.world_state
        except ImportError:
            pass
        ...
        for interceptor_class in InterceptorBase.__subclasses__():
            if (
                not inspect.isabstract(interceptor_class)
                and interceptor_class.__name__ != "InterceptorBase"
            ):
                if interceptor_class in seen_classes:
                    continue
                seen_classes.add(interceptor_class)
                try:
                    instances.append(interceptor_class())
                except Exception as exc:
                    logger.warning(
                        f"Failed to auto-register interceptor {interceptor_class.__name__}: {exc}"
                    )
...
class _InterceptorCache:
    ...
    def set(
        self, agent_id: str, key: str, value: str, ttl: Optional[float] = None
    ) -> None:
        with self._lock:
    ...
    def invalidate(self, agent_id: str, key: Optional[str] = None) -> None:
        ...
        with self._lock:
            if key:
                ...
                self._store = {k: v for k, v in self._store.items() if k[0] != agent_id}
...
class RouterMessageInjector(InterceptorBase):
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        logger.debug(
            "RouterMessageInjector: injected %d message(s) for %s",
            len(pending),
            agent_id,
        )
...
class SkillAwarenessInterceptor(InterceptorBase):
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        if required_skills:
            ...
            descs = "\n".join(f"  • {s.name}: {s.description}" for s in required_skills)
            parts.append(
        ...
        if optional_skills:
            descs = "\n".join(f"  • {s.name}: {s.description}" for s in optional_skills)
            parts.append(f"AVAILABLE TOOLS (use when relevant):\n{descs}")
...
class ConversationVarietyInterceptor(InterceptorBase):
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        try:
            ...
            logger.debug(
                "ConversationVarietyInterceptor: heat directive failed: %s", exc
            )
        ...
        if agent_id and reply:
            with self._recent_lock:
                ...
                if len(self._recent_responses[agent_id]) > self._MAX_TRACKED:
                    self._recent_responses[agent_id] = self._recent_responses[agent_id][
                        -self._MAX_TRACKED :
                    ]
            ...
            try:
                ...
                logger.debug(
                    "ConversationVarietyInterceptor: heat analysis failed: %s", exc
                )
...
class BedroomSceneInterceptor(InterceptorBase):
    ...
    def pre_call(self, ctx: ResponseContext) -> None:  # noqa: D401
        ...
        char_ids: List[str] = ctx.get("character_ids") or (
        ...
        try:
            ...
            narrative_block = (
                "\n".join(f"  • {e}" for e in narrative)
                if narrative
                else "  (scene just started)"
            )
            ...
            if atm:
                parts = []
                if atm.get("lighting"):
                    parts.append(f"lighting={atm['lighting']}")
                if atm.get("mood"):
                    parts.append(f"mood={atm['mood']}")
                if atm.get("music"):
                    parts.append(f"music={atm['music']}")
            ...
            try:
                ...
                for cid in char_ids:
                    ...
                    available = eng.get_available_actions(
                        BEDROOM_SCENE_ID, cid, stats_dict
                    )
                    if available:
                        acts = ", ".join(
            ...
            try:
                ...
                logger.debug(
                    "BedroomSceneInterceptor: timed action phase failed: %s", exc
                )
...
class PhoneSceneInterceptor(InterceptorBase):
    ...
    _VIBE_HINTS: Dict[tuple, str] = {
        (
            "high",
            "high",
        ): "You are intensely engaged — flirty, forward, a little breathless.",
        (
            "high",
            "mid",
        ): "You feel the heat rising but still hold a hint of playful restraint.",
        (
            "high",
            "low",
        ): "You're aroused but guarded — mixed feelings, simmering tension.",
        (
            "mid",
            "high",
        ): "You're comfortable and warm, happy to lean into wherever this goes.",
        ("mid", "mid"): "You're your usual self — curious, a little flirty, easy.",
        (
            "mid",
            "low",
        ): "You're present but not open to anything too intense right now.",
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        try:
            ...
            try:
                ...
                if directive:
                    lines.append(
                        f"[CONVERSATION HEAT: {heat_level:.0f}/100] {directive}"
                    )
                    if heat_level < 30:
                        lines.append(
            ...
            try:
                ...
                if agent_id:
                    ...
                    available = eng.get_available_actions("phone", agent_id, stats_dict)
                    if available:
...
class LoungeSceneInterceptor(InterceptorBase):
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        try:
            ...
            trust = int(
                (guest_stats.trust if guest_stats else 0)
                or lola_state.get("guest_trust", 10)
            )
            ...
            try:
                ...
                actions = eng.get_available_actions(SCENE_ID, agent_id, stats_dict)
                available_actions = [a["id"] for a in actions[:8]]
            except Exception as exc:
                logger.debug(
                    "LoungeSceneInterceptor: available actions failed: %s", exc
                )
            ...
            try:
                ...
                logger.debug(
                    "LoungeSceneInterceptor: cross-scene inbox failed: %s", exc
                )
...
class UniversalSceneInterceptor(InterceptorBase):
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        try:
            ...
            actions = eng.get_available_actions(scene, agent_id, stats_dict)
            if actions:
...
class CharacterRegistryInterceptor(InterceptorBase):
            ...
            if personality_id:
                ...
                if profile:
                    ...
                    if profile.get("traits"):
                        traits = (
                            profile["traits"]
                            if isinstance(profile["traits"], str)
                            else ", ".join(profile["traits"])
                        )
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        try:
            ...
            if summary:
                ...
                if summary.get("restrictions"):
                    lines.append(
                        f"Current restrictions: {', '.join(summary['restrictions'])}"
                    )
                ...
                if cached_profile is None:
                    ...
                    if cached_profile:
                        INTERCEPTOR_CACHE.set(
                            agent_id, "personality_profile", cached_profile, ttl=300.0
                        )
                if cached_profile:
        ...
        try:
            ...
            logger.debug(
                "CharacterRegistryInterceptor framework enter_scene failed: %s", exc
            )
        ...
        try:
            ...
            if directive and directive.get("directive_type") == "force_response":
                ...
                if forced:
                    ...
                    logger.debug(
                        "CharacterRegistryInterceptor: force_response applied for %s",
                        agent_id,
                    )
        except Exception as exc:
        ...
        try:
            ...
            if not voice_id and agent_id:
                try:
                    ...
                    logger.debug(
                        "TTSStyleInterceptor: voice config lookup failed: %s", exc
                    )
        except Exception as exc:
    ...
    def pre_call(self, ctx: ResponseContext) -> None:
        ...
        try:
            ...
            if arousal > 70:
                ...
                if k in self._DRIFT and isinstance(v, (int, float))
            ):
                thought = self._INNER_THOUGHTS["mellowing"]

            if thought:
                ctx["system_prompt"] = (
                    ctx.get("system_prompt", "") + f"\n\n[Inner feeling: {thought}]"
                )
...
class ConversationRecapInterceptor(InterceptorBase):
    ...
    def _conv_key(self, ctx: ResponseContext) -> str:
        return (
            ...
            or f"{ctx.get('scene', 'default')}_{ctx.get('agent_id', 'anon')}"
        )
...
class NexusPromptInterceptor(InterceptorBase):
    ...
    def _cached_fetch(self, key: str, fetcher: Any) -> Any:
        ...
        with self._lock:
            if (
                key in self._cache
                and (now - self._cache_ts.get(key, 0)) < self._TTL_SECS
            ):
