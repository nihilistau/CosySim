...
class TestCharacterState:
    def test_defaults(self):
        ...
        assert isinstance(s.restrictions, set)

    def test_to_dict(self):
        s = CharacterState(mood="happy", restrictions={"no_violence", "keep_calm"})
        d = s.to_dict()
        assert d["mood"] == "happy"
        assert d["restrictions"] == ["keep_calm", "no_violence"]  # sorted
...
class TestRegistryRegister:
    ...
    def test_register_adds_character(self):
        ...
        assert rec.profile.name == "Aria"

    def test_register_returns_record(self):
        rec = _register_aria(self.reg)
        assert isinstance(rec, CharacterRecord)
    ...
    def test_duplicate_register_replaces_profile_keeps_state(self):
        ...
        assert rec2.profile.name == "Aria V2"
        assert rec2.state.mood == "excited"  # preserved
    ...
    def test_duplicate_register_preserves_skills(self):
        ...
        assert self.reg.has_skill("aria", "mem")
...
class TestRegistryProfileQueries:
    def setup_method(self):
        self.reg = _fresh_registry()
        _register_aria(self.reg)
    ...
    def test_get_profile_unknown_returns_none(self):
        assert self.reg.get_profile("nobody") is None

    def test_get_attribute_appearance_key(self):
        assert self.reg.get_attribute("aria", "eyes") == "green"

    def test_get_attribute_field(self):
        assert self.reg.get_attribute("aria", "age") == 26

    def test_get_attribute_unknown_char(self):
        assert self.reg.get_attribute("nobody", "eyes") is None
    ...
    def test_personality_access(self):
        p = self.reg.get_profile("aria")
        assert p.personality["warmth"] == pytest.approx(0.9)
...
class TestRegistryRestrictions:
    def setup_method(self):
        self.reg = _fresh_registry()
        _register_aria(self.reg)
    ...
    def test_restrictions_appear_in_state_dict(self):
        ...
        state = self.reg.get_state("aria")
        assert "no_violence" in state["restrictions"]
...
class TestRegistrySkills:
    ...
    def test_assign_and_get_skill(self):
        ...
        assert skill.params["top_k"] == 5

    def test_has_skill(self):
        self.reg.assign_skill("aria", "mem", skill_type="memory_recall")
        assert self.reg.has_skill("aria", "mem")
        assert not self.reg.has_skill("aria", "nonexistent")

    def test_revoke_skill(self):
        self.reg.assign_skill("aria", "mem", skill_type="memory_recall")
        removed = self.reg.revoke_skill("aria", "mem")
        assert removed
        assert not self.reg.has_skill("aria", "mem")

    def test_revoke_nonexistent_returns_false(self):
        assert not self.reg.revoke_skill("aria", "nope")

    def test_toggle_skill(self):
        self.reg.assign_skill("aria", "mem", skill_type="memory_recall")
        self.reg.toggle_skill("aria", "mem", enabled=False)
        skills = self.reg.get_skills("aria", enabled_only=True)
        assert not any(s.skill_id == "mem" for s in skills)

        self.reg.toggle_skill("aria", "mem", enabled=True)
        skills = self.reg.get_skills("aria", enabled_only=True)
        assert any(s.skill_id == "mem" for s in skills)
    ...
    def test_get_skills_sorted_by_priority(self):
        ...
        assert skills[0].skill_id == "b"  # lower priority = first
...
class TestRegistryRecordAndSummary:
    def setup_method(self):
        self.reg = _fresh_registry()
        _register_aria(self.reg)
    ...
    def test_get_record_unknown_returns_none(self):
        assert self.reg.get_record("ghost") is None
    ...
    def test_get_character_summary(self):
        ...
        assert summary["mood"] == "excited"
        assert "mem" in summary["active_skills"]
        assert "warmth" in summary["top_traits"]

    def test_ensure_creates_stub(self):
        rec = self.reg.ensure("new_char")
        assert rec is not None
        assert rec.profile.name == "New Char"
...
class TestRegistryLoadFromDict:
    def test_load_from_dict(self):
        ...
        assert rec.profile.name == "Lena"
        assert rec.profile.appearance["hair"] == "blonde"
        state = reg.get_state("lena")
        assert state["mood"] == "happy"
        assert reg.has_skill("lena", "web")
...
class TestSingleton:
    ...
    def test_singleton_has_framework_character(self):
        ...
        assert "__framework__" in reg.list_characters()
